<template>
  <div class="bg-[#060a18] text-white antialiased selection:bg-cyan-400/30">
    ...
    <main>
      <!-- HERO / INTRO -->
      <section id="intro" class="gradient-hero relative overflow-hidden">
          ...
          <div class="grid gap-10 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,420px)] lg:items-center">
            <div class="space-y-6">
              ...
              <h1 class="text-3xl sm:text-5xl md:text-6xl font-semibold leading-tight">
                Mach mit bei OpenSquawk – Early Access mit Wirkung.
              </h1>
              <p class="text-white/80 text-base sm:text-lg">
                OpenSquawk ist eine offene, kosteneffiziente AI-ATC-Plattform für Flugsimulatoren.
                Gebaut mit Nuxt&nbsp;4 und einem Node.js-Backend vereinen wir Training, Funk und Lerninhalte für Simulatorpiloten
                – mit dem Ziel, eine community-getriebene Alternative zu teuren Closed-Source-Lösungen zu bieten.
              </p>
              <div class="grid gap-3 sm:grid-cols-2">
                <div class="rounded-2xl border border-white/10 bg-white/5 p-4 space-y-2">
                  <div class="text-xs uppercase tracking-[0.3em] text-white/50">Projekt heute</div>
                  <ul class="space-y-2 text-sm text-white/70">
                    ...
                    >
                      <v-icon :icon="highlight.icon" size="18" class="mt-[2px] text-cyan-300" />
                      <span>{{ highlight.text }}</span>
                    </li>
                  </ul>
                </div>
                <div class="rounded-2xl border border-cyan-400/30 bg-cyan-500/10 p-4 space-y-2">
                  <div class="text-xs uppercase tracking-[0.3em] text-cyan-200">Programm-Highlights</div>
                  <ul class="space-y-2 text-sm text-white/80">
                    <li
                      v-for="highlight in programHighlights"
                      :key="highlight.text"
                      class="flex items-start gap-2"
                    >
                      <v-icon :icon="highlight.icon" size="18" class="mt-[2px] text-cyan-200" />
                      <span>{{ highlight.text }}</span>
                    </li>
                  </ul>
                </div>
              ...
              <div class="flex flex-col gap-2.5 sm:flex-row sm:items-center sm:gap-3">
                ...
                <a :href="README_URL" target="_blank" rel="noopener" class="btn btn-ghost">
      ...
      <!-- PROJECT OVERVIEW -->
      <section id="project" class="py-12 sm:py-16 md:py-24 bg-[#070c1c] border-t border-white/10">
        <div class="container-outer space-y-10">
          <div class="max-w-3xl space-y-4">
            <span class="eyebrow">Projektüberblick</span>
            <h2 class="text-3xl md:text-4xl font-semibold">Was ist OpenSquawk?</h2>
            <p class="text-white/80">
              Die Plattform kombiniert Web-Interface, ATC-Logik, Sprachmodelle und Lerncontent.
              Wir entwickeln transparent: Self-Hosting bleibt möglich, Hosted-Angebote kommen später.
            </p>
          </div>
          <div class="grid gap-4 md:grid-cols-3 md:gap-6">
            <article
              v-for="card in projectOverviewCards"
              :key="card.title"
              class="card h-full space-y-3"
            >
              <div class="flex items-center gap-3">
                <div class="p-2 rounded-xl border border-cyan-400/30 bg-cyan-500/10">
                  <v-icon :icon="card.icon" size="24" class="text-cyan-200" />
                </div>
...
const README_URL = 'https://github.com/FaktorxMensch/OpenSquawk#readme'
...
const navLinks: NavLink[] = [
  {label: 'Überblick', to: '#intro'},
  {label: 'Projekt', to: '#project'},
...
const mobileNavLinks = computed<ExtendedNavLink[]>(() => [
  {label: 'Landing Page', to: '/', icon: 'mdi-home-outline'},
  ...navLinks,
  {label: 'Kontakt & Assets', to: '#cta'},
  {label: 'README', to: README_URL, external: true, icon: 'mdi-book-open-variant'},
...
const projectHighlights: Highlight[] = [
  {icon: 'mdi-airplane-clock', text: 'Alpha-Status: Fokus auf Microsoft Flight Simulator, X-Plane folgt.'},
  {icon: 'mdi-web', text: 'Modernes Web-UI (Nuxt 4, Tailwind, Vuetify) für Landing & Learning.'},
  {icon: 'mdi-robot-voice', text: 'OpenAI-basierte Funk- & Sprachmodelle plus optionaler Piper-TTS-Server.'},
]

const programHighlights: Highlight[] = [
  {icon: 'mdi-vote-outline', text: 'Scorecards & Votes entscheiden Features innerhalb von 72 Stunden.'},
  {icon: 'mdi-shield-star-outline', text: 'Rewards & Badges binden sich automatisch an Discord-Rollen.'},
  {icon: 'mdi-format-list-checks', text: 'Transparente Leaderboards – jede Aktion erhält Label & Review.'},
]
...
const projectOverviewCards: ProjectOverviewCard[] = [
  {
    title: 'Aktuelle Features',
    icon: 'mdi-lightning-bolt-outline',
    description: 'Bereits live im Alpha-Test – alles open-source und iterativ erweiterbar.',
    items: [
      'Modernes Web-Interface mit Landing- & Lernsektionen.',
      'Server-APIs für ATC-Logik, Trainingscontent & Authentifizierung.',
      'MongoDB 7+ als persistente Basis.',
      'OpenAI-TTS/LLM plus optionaler Speeches-Server.',
    ],
  },
  {
    title: 'Roadmap & Integrationen',
    icon: 'mdi-timeline-clock-outline',
    description: 'Integrationen sind vorbereitet und werden wellenweise mit der Community getestet.',
    items: [
      'Simulator-Bridges zu MSFS & X-Plane (Flight-Data ingest).',
      'Direkter SimBrief-Import für Briefings & Dispatch.',
      'VATSIM-Beobachtung – Integration sobald Lizenzlage geklärt ist.',
    ],
  },
  {
    title: 'Setup & Requirements',
    icon: 'mdi-toolbox-outline',
    description: 'Schnell startklar – lokale oder gehostete Infrastruktur nach Wahl.',
    items: [
      'Node.js 22 & Yarn laut package.json.',
      'MongoDB 7+, ffmpeg und ein OpenAI API Key.',
      'Optional: Piper TTS für lokale Sprachausgabe.',
      'Umgebungsvariablen via .env Beispiel konfigurierbar.',
    ],
  },
]
...
useHead(() => ({
  htmlAttrs: {lang: 'de'},
  title: 'OpenSquawk Contributor Program – Projekt & Early Access',
  meta: [
    {
      name: 'description',
      content:
        'Projektüberblick aus der README plus Contributor Program: Early-Access-Wellen, Co-Creation, Credits und Rewards.',
    },
    {name: 'theme-color', content: '#0ea5e9'},
    {property: 'og:title', content: 'OpenSquawk Contributor Program'},
    {
      property: 'og:description',
      content:
        'OpenSquawk erklärt: Setup, Roadmap und Community-Programm mit Punkten, Leaderboard und dauerhaften Credits.',
    },
    {property: 'og:type', content: 'website'},
    {property: 'og:image', content: 'https://opensquawk.example.com/cover.png'},
    {name: 'twitter:card', content: 'summary_large_image'},
    {name: 'twitter:title', content: 'OpenSquawk Contributor Program'},
    {
      name: 'twitter:description',
      content:
        'Mach mit im Community-Pilot-Programm: Alpha/Beta-Wellen, Co-Creation-Streams, Credits und Rewards im Überblick.',
    },
