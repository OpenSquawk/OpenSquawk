                ...
                <div class="mt-6 space-y-4">
                  ...
                  >
                    ...
                    <div class="flex flex-wrap items-center gap-3">
                      <button type="button" class="btn primary" @click="handleRadioCheck" :disabled="speechLoading">
                        <v-icon
      ...
      <section id="orientation" class="border-b border-white/5 bg-[#080f1f]/80 py-16 sm:py-20">
        <div class="mx-auto flex w-full max-w-screen-xl flex-col gap-10 px-4 sm:px-6 md:px-8 lg:flex-row">
          <aside class="shrink-0 rounded-3xl border border-white/10 bg-white/5 p-6 shadow-lg shadow-black/20 lg:w-80">
            ...
            <button v-if="tourStarted" type="button" class="btn soft mt-6" @click="restartTour">
              ...
              >
                ...
                <div class="space-y-6 px-6 py-8 sm:px-8">
                    ...
                    <div class="flex items-start gap-4">
                      ...
                      <div class="flex-1 space-y-3">
                        ...
                        <div v-if="voiceMode === 'radio'" class="flex flex-wrap items-center gap-3">
                          <button type="button" class="btn soft mini" @click="replayStageVoice" :disabled="speechLoading">
                            <v-icon
                  ...
                  <div class="border-t border-white/10 pt-6">
                    <div class="flex flex-wrap items-center gap-3">
                      <button v-if="canGoPrevious" type="button" class="btn ghost" @click="goToPrevious">
                      ...
                      <div class="ml-auto flex flex-wrap items-center gap-3">
                        <button
                          v-if="voiceMode === 'radio'"
                          type="button"
                          class="btn ghost mini uppercase tracking-[0.2em]"
...
import { loadPizzicatoLite, type PizzicatoLite, type PizzicatoSound } from '~~/shared/utils/pizzicatoLite'
...
let activeRadioSound: PizzicatoSound | null = null
...
async function playWithRadioEffects(base64: string, requestId: number): Promise<boolean> {
  ...
  const readability = Math.max(1, Math.min(5, radioLevel.value))
...
async function playPlainAudio(base64: string, mime: string, requestId: number): Promise<boolean> {
  if (typeof window === 'undefined') {
    return false
  }
...
async function speak(text: string, stageId?: string): Promise<boolean> {
  ...
  try {
    ...
    let playedWithEffect = false
    try {
      playedWithEffect = await playWithRadioEffects(base64, requestId)
    } catch (error) {
      console.warn('Falling back to clean audio playback', error)
    }
    ...
    return await playPlainAudio(base64, mime, requestId)
  } catch (error: any) {
    if (speechRequestId === requestId) {
...
.btn:hover:not([disabled]) {
  ...
  color: #ffffff;
  box-shadow: 0 12px 26px rgba(8, 13, 26, 0.35);
...
.btn.primary {
  ...
  color: #061318;
  box-shadow: 0 12px 28px rgba(13, 148, 233, 0.25);
}

.btn.primary:hover:not([disabled]) {
  background: linear-gradient(180deg, rgba(34, 211, 238, 0.9), rgba(14, 165, 233, 0.8));
}
