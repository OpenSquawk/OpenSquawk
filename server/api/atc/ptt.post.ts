// server/api/atc/ptt.post.ts — STT only (v2: decision routing happens client-side via /api/atc/route)
...
export default defineEventHandler(async (event) => {
    ...
    if (!body.audio) {
        throw createError({ statusCode: 400, statusMessage: "audio is required" });
    }
