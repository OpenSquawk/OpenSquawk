...
export default function useCommunicationsEngine() {
    ...
    function buildLLMContext(pilotTranscript: string) {
        ...
        const candidates = nextCandidates.value
            .map(id => {
                ...
                return { id, flow: activeFlowSlug.value || undefined, state }
            })
            .filter((candidate): candidate is { id: string; flow?: string; state: RuntimeDecisionState } => Boolean(candidate))

        const flowState: Record<string, string> = {}
        for (const slug of flowOrder.value) {
            const snapshot = flowSnapshots[slug]
            if (snapshot?.currentStateId) {
                flowState[slug] = snapshot.currentStateId
                continue
            }
            const treeData = runtimeSystem.value?.flows?.[slug]
            if (treeData?.start_state) {
                flowState[slug] = treeData.start_state
            }
        }
        if (activeFlowSlug.value && currentStateId.value) {
            flowState[activeFlowSlug.value] = currentStateId.value
        }
        ...
        return {
            ...
            tree: runtime.name,
            active_flow: activeFlowSlug.value || runtime.slug || runtime.name,
            flow_state: flowState,
            flow_order: [...flowOrder.value],
    ...
    function applyLLMDecision(decision: any) {
        ...
        const nextFlow = typeof decision.next_flow === 'string' ? decision.next_flow : null

        if (resumeFlow) {
            resumePriorFlow()
        } else if (!decision.radio_check && nextState) {
            if (nextFlow && nextFlow !== activeFlowSlug.value) {
                try {
                    setActiveFlow(nextFlow)
                } catch (error) {
                    console.error('Failed to switch decision flow', error)
                }
            }
            moveTo(nextState)
        }
